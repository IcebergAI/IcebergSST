...
| `/findings`, `/findings/{id}` | Queue, detail, triage, remediation panel | `GET/PATCH /findings`, `/remediation/guidance/{rule_id}`, `/findings/{id}/remediations…` (ADR 0011) | viewer / analyst to triage or record |
| `/clusters`, `/clusters/{id}` | Exposure clusters: spread view, topology, export link | `GET /correlation/clusters…` (ADR 0010) | analyst |
